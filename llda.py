...
https://github.com/fann1993814/llda
'''
...
class LLDA():
    ...
    def __init__(self, alpha, eta, num_topics, vocab_size=0, num_docs=0):
        ...
        self.corpus_model = CountVectorizer()
    ...
    def random_initialization(self, corpus, label_matrix):
        ...
        for i, doc in enumerate(corpus):
            ...
            for j in word_inds:
                # Choose a random (valid) topic assignment
                doc_labels = self.label_matrix[i]
                rand_topic = np.random.choice(np.argwhere(doc_labels == 1).reshape(-1,))
    ...
    def fit(self, max_iters=1000):
        ''' An implementation of the collapsed Gibb's Sampler for LLDA. '''
        lls = []
        for iters in range(0, max_iters):
            ...
            for i in range(0, self.num_docs):
                ...
                for j in word_inds:
                    ...
                    labels = np.argwhere(self.label_matrix[i] == 1).T[0]
                    log_prob = np.log(self.n_wk[j, labels] + self.eta)
                    log_prob -= np.log(self.n_k[labels] + self.eta * self.vocab_size)
                    log_prob += np.log(self.n_dk[i, labels] + self.alpha) # TODO try making topic cut here
