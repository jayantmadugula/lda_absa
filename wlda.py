...
class MLDA():
    ''' Implementation of changed LLDA with a Collapsed Gibb's Sampler for posterior inference. This model puts a prior on the word topic distribution instead of 
    ...
    def fit(self, max_iters=1000):
        ''' An implementation of the collapsed Gibb's Sampler. '''
        lls = []
        for iters in range(0, max_iters):
            ...
            for i in range(0, self.num_docs):
                ...
                for j in word_inds:
                    weights = (self.label_matrix[j] == 1).astype(float)
                    ...
                    prob = np.log(np.exp(log_prob) * weights)
