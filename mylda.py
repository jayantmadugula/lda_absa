...
class MLLDA():
    ''' Implementation of LLDA with a Collapsed Gibb's Sampler for posterior inference. Allows weights on documents instead of just binary labels. '''
    ...
    def fit(self, max_iters=1000):
        ...
        for iters in range(0, max_iters):
            ...
            for i in range(0, self.num_docs):
                word_inds = np.nonzero(self.X[i])[1]
                labels = (self.label_matrix[i] == 1).astype(float)
                ...
                for j in word_inds:
                    ...
                    prob = np.log(np.exp(log_prob) * labels)
